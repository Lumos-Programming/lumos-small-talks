...
export async function updateDiscordEvent(
  ...
  if (params.scheduledStartTime) body.scheduled_start_time = params.scheduledStartTime
  if (params.scheduledEndTime) body.scheduled_end_time = params.scheduledEndTime
  if (params.location) {
  ...
  if (!response.ok) {
    const error = await response.text()
    throw new Error(`Failed to update Discord event: ${response.status} ${error}`)
  }
...
export async function deleteDiscordEvent(eventId: string): Promise<void> {
  ...
  if (!response.ok) {
    const error = await response.text()
    throw new Error(`Failed to delete Discord event: ${response.status} ${error}`)
  }
}
